...
def fit_logistic(config: dict | None = None):
    ...
    with open(processed_dir / "logistic_model.pkl", "wb") as f:
        pickle.dump(
            {
    ...
    train_out.to_csv(processed_dir / "logistic_train_predictions.csv", index=False)
    test_out.to_csv(processed_dir / "logistic_test_predictions.csv", index=False)
    np.savetxt(processed_dir / "logistic_confusion_matrix.csv", cm, delimiter=",", fmt="%d")

    return sk_model, mnlogit, train_out, test_out, cm, aucs
